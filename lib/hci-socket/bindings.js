...
NobleBindings.prototype.disconnect = function(peripheralUuid) {
  // if not handle, no connection but the command for connection can be still there
  if (!this._handles[peripheralUuid]) {
    // Cancel connection if ongoing
    if (this._pendingConnectionUuid === peripheralUuid) {
      // set a cancel command a stop, the connection will be close with error 0x02
      this.cancelLeConn()
      return
    }
    // Check if device in queue for connection and remove
    let idx = this._connectionQueue.indexOf(peripheralUuid)
    if (idx >= 0) {
      this._connectionQueue.splice(idx,1)
    }
    // sent disconnect to report no connection
...
NobleBindings.prototype.onLeConnComplete = function(status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
  ...
  this.emit('connect', uuid, error, handle);
