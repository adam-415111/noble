...
Noble.prototype.onConnect = function(peripheralUuid, error, handle) {
  var peripheral = this._peripherals[peripheralUuid];

  if (peripheral) {
    // Check if connection come with error 0x02. Happens when sending cancel connection command
    // TODO Come error 0x2 in other cases?
    if (error && error.stack &&  error.stack.includes('(0x2)')) {
      peripheral.state = 'disconnected'
      peripheral.emit('disconnect');
    } else {
      peripheral.state = error ? 'error' : 'connected';
      peripheral.emit('connect', error, handle);
    }
  } else {
...
Noble.prototype.onCharacteristicsDiscovered = function(peripheralUuid, serviceUuid, characteristics) {
  var service = this._services[peripheralUuid][serviceUuid];
console.log('ONCHAR ' + service + ' ' + JSON.stringify(characteristics));
